...
export class MtlParser {
    ...
    setNumericProperty(propertyName, parts, invert = false) {
        if (!this.currentMaterial)
            return;
        const value = parseFloat(parts[1]);
        if (!isNaN(value)) {
            this.currentMaterial[propertyName] = invert ? (1.0 - value) : value;
        }
    }
    setColorProperty(propertyName, parts) {
        if (!this.currentMaterial)
            return;
        ...
        if (!isNaN(r) && !isNaN(g) && !isNaN(b)) {
            this.currentMaterial[propertyName] = vec4.fromValues(r, g, b, 1.0);
        }
    }
    setTextureMapProperty(propertyName, parts) {
        if (!this.currentMaterial)
            return;
        // Texture map path is the rest of the line, after the prefix
        const path = parts.slice(1).join(' '); // Re-join if path has spaces
        if (path) {
            this.currentMaterial[propertyName] = path;
        }
    }
}
