...
export class ObjParser {
    constructor() {
        ...
        this.outputNormals = [];
        this.outputUVs = [];
        this.outputIndices = []; // Indices for drawing (e.g., in WebGL)
    }
    /**
    ...
    parse(objContent) {
        ...
        return new MeshData(this.outputVertices, this.outputNormals, this.outputUVs, this.outputIndices);
    }
    resetParser() {
        this.tempVertices = [];
        this.tempNormals = [];
    ...
    parseVertex(parts) {
        // 'v x y z'
        this.tempVertices.push(parseFloat(parts[1]), parseFloat(parts[2]), parseFloat(parts[3]));
    }
    parseUV(parts) {
        // 'vt u v' (or 'vt u v w', w is often 0 or 1, we usually only care about u,v)
        this.tempUVs.push(parseFloat(parts[1]), parseFloat(parts[2]));
    }
    parseNormal(parts) {
        // 'vn nx ny nz'
        this.tempNormals.push(parseFloat(parts[1]), parseFloat(parts[2]), parseFloat(parts[3]));
    }
    parseFace(parts) {
        // 'f v1/vt1/vn1 v2/vt2/vn2 v3/vt3/vn3 ...'
        // Can also be 'f v1 v2 v3' or 'f v1/vt1 v2/vt2' or 'f v1//vn1 v2//vn2'
        const face = { positions: [], uvs: [], normals: [] };
    ...
    consolidateMeshData() {
        const uniqueVertexMap = new Map(); // Maps "v_idx/vt_idx/vn_idx" to new flat index
        ...
        for (const face of this.tempFaces) {
            for (let i = 0; i < 3; i++) { // Each triangulated face has 3 vertices
                ...
                if (!uniqueVertexMap.has(key)) {
                    ...
                    // Add position data
                    this.outputVertices.push(vec3.fromValues(this.tempVertices[posIdx * 3 + 0], this.tempVertices[posIdx * 3 + 1], this.tempVertices[posIdx * 3 + 2]));
                    // Add UV data (handle if not present in OBJ)
                    if (uvIdx !== -1) {
                        this.outputUVs.push(vec2.fromValues(this.tempUVs[uvIdx * 2 + 0], this.tempUVs[uvIdx * 2 + 1]));
                    }
                    else {
                    ...
                    if (normIdx !== -1) {
                        this.outputNormals.push(vec3.fromValues(this.tempNormals[normIdx * 3 + 0], this.tempNormals[normIdx * 3 + 1], this.tempNormals[normIdx * 3 + 2]));
                    }
                    else {
